...
if ( ! class_exists( 'WCVendors_Admin_Post_Types', false ) ) :

/**
 * WCVendors_Admin_Post_Types Class.
 *
 * Hook into the WooCommerce custom post types and add vendor and marketplace extensions 
 */
class WCVendors_Admin_Post_Types {
	...
	public function pending_items_count( $menu ) {
		...
		foreach( $menu as $menu_key => $menu_data ) {
			if ( 'users.php' === $menu_data[2] && ! empty( $pending_vendors_count ) ) {
				$menu[$menu_key][0] .= ' <span class="update-plugins count-' . $pending_vendors_count . '" title="' . esc_attr__( 'Products awaiting review', 'wcvendors' ) . '"><span class="plugin-count">' . number_format_i18n( $pending_vendors_count ) . '</span></span>';
			}
			if ( 'edit.php?post_type=product' === $menu_data[2] && ! empty( $products->pending ) ) {
				$menu[$menu_key][0] .= ' <span class="update-plugins count-' . $pending_products_count . '" title="' . esc_attr__( 'Products awaiting review', 'wcvendors' ) . '"><span class="plugin-count">' . number_format_i18n( $pending_products_count ) . '</span></span>';
			}
		}
		return $menu;
	}
	...
	*/	
	public function quick_edit( $column_name, $post_type ) { 
		...
		include( WCV_ABSPATH . 'includes/admin/views/html-admin-quick-edit-product.php' );
	} 
	...
	*/	
	public function bulk_edit( $column_name, $post_type ) { 
		...
		include( WCV_ABSPATH . 'includes/admin/views/html-admin-bulk-edit-product.php' );
	} 
	...
	public function bulk_and_quick_edit_save_post( $post_id, $post ) {
		...
		if ( isset( $_REQUEST['wcvendors_quick_edit_nonce'] ) && ! wp_verify_nonce( $_REQUEST['wcvendors_quick_edit_nonce'], 'wcvendors_quick_edit_nonce' ) ) {
			return $post_id;
		}
		if ( isset( $_REQUEST['wcvendors_bulk_edit_nonce'] ) && ! wp_verify_nonce( $_REQUEST['wcvendors_bulk_edit_nonce'], 'wcvendors_bulk_edit_nonce' ) ) {
			return $post_id;
		}
...
return new WCVendors_Admin_Post_Types(); 