jQuery(function($) {
	...
	$('#the-list').on('click', '.editinline', function() {
		if (
			$('.inline-edit-author').length &&
			$('.inline-edit-author-vendor').length
		) {
		...
		// Set values
		$(
			'select[name="post_author"] option[value="' + vendor_id + '"]',
			'.inline-edit-row'
		).attr('selected', 'selected');
		$('input[name="_wcv_commission_rate"]', '.inline-edit-row').val(
			commission_rate
		);
	});
