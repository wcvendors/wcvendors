...
trait Core {
	 ...
	 *
	 * @param string $order_status Order status string id.
	 *
	 * @return string
	 */
	public function format_order_status( $order_status = '' ) {
		switch ( $order_status ) {
			case 'pending':
	 ...
	 * @since 1.0.0
	 */
	public function get_date_from_gmt( $string, $format, $timezone_string ) {
		if ( ! $format ) {
			$format = 'Y-m-d H:i:s';
		}
		...
		if ( $timezone_string && ( ! preg_match( '/UTC-/', $timezone_string ) && ! preg_match( '/UTC+/', $timezone_string ) ) ) {
			$datetime = date_create( $string, new DateTimeZone( 'UTC' ) );

			if ( ! $datetime ) {
				return date( $format, 0 ); // phpcs:ignore
			}
	 ...
	 *
	 * @param string $sql_date SQL date to be formatted.
	 ...
	 * @version 1.0.0
	 */
	public function format_date( $sql_date, $timezone = '' ) {
